...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--register-settings",
        action="store_true",
        help="Also insert missing include(...) entries into settings.gradle(.kts) idempotently",
    )
...
def existing_includes(text: str) -> set[str]:
    found: set[str] = set()
    for match in re.finditer(r'include\s*\(([^)]*)\)', text, re.DOTALL):
...
def register_settings(root: Path, modules: list[str], apply: bool) -> tuple[Path | None, list[str], str | None]:
    settings = find_settings_file(root)
    if settings is None:
        return None, modules, "no settings.gradle(.kts) found; print-only includes"
    text = settings.read_text(encoding="utf-8")
    present = existing_includes(text)
    missing = [module for module in modules if module not in present]
    if not missing:
        return settings, [], None
    include_line = "include(" + ", ".join(f'"{module}"' for module in missing) + ")\n"
    if not apply:
        return settings, missing, include_line
...
def main() -> int:
    ...
    print("\nSettings includes (deterministic order):")
    print(f"  include({include_args})")
    if options.register_settings:
        settings, missing, line = register_settings(root, modules, apply=False)
        if settings is None:
            print("  warning: no settings.gradle(.kts) present; cannot auto-register")
        elif not missing:
            print(f"  {settings.relative_to(root)}: all includes already present (idempotent)")
        else:
            print(f"  {settings.relative_to(root)}: will append missing includes:")
            for module in missing:
                print(f"    - {module}")
    else:
        print("  (pass --register-settings to insert missing includes; default is print-only)")
    ...
    if options.register_settings:
        settings, missing, _ = register_settings(root, modules, apply=True)
        if settings is None:
            print("warning: modules scaffolded but settings file was not found")
        elif missing:
            print(f"Registered {len(missing)} module(s) in {settings.relative_to(root)}")
        else:
            print(f"Settings already contained all includes in {settings.relative_to(root)}")
    else:
